...
function startServer() {
  ...
  app.post(routePrefix + "/register", async (req, res) => {
    ...
    if (!login || !email || !password) {
      ...
      res.status(400).json({
        status: "error",
        message: "Some field not found { login, email, password }",
      });
    } else {
      try {
        ...
        conn
          ...
          .then((v) => {
            ...
            var token = jwt.sign({ login: login, email: email }, privateKey, {
              algorithm: "RS256",
              expiresIn: jwtLifeTime,
            });
            res.status(201).json({ status: "ok", jwt: token });
          })
          .catch((err) => {
            ...
            if (err.code === "ER_DUP_ENTRY") {
              ...
              res.status(400).json({
                status: "error",
                message: "already user login or email",
              });
            } else {
              console.log("user not created", err);
              res
                .status(400)
                .json({ status: "error", message: "unspecified backend err" });
            }
          })
          .finally(() => {
            conn.release();
  ...
  app.post(routePrefix + "/login", async (req, res) => {
    ...
    if (!login || !password) {
      ...
      try {
        ...
        conn
          .query(
            "select passwordHash,email from users where login = ? limit 1",
            [login]
          )
          .then((v) => {
            if (v.length == 1) {
              ...
              const encryptedPassword = crypto
                .pbkdf2Sync(password, knownHash[0], 1000, 64, "sha512")
                .toString("hex");
              if (encryptedPassword === knownHash[1]) {
                var token = jwt.sign(
                  { login: login, email: user.email },
                  privateKey,
                  { algorithm: "RS256", expiresIn: jwtLifeTime }
                );
